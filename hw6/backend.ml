...
let better_layout (f:Ll.fdecl) (live:liveness) : layout =
  let palette = [Rdx; Rsi; Rdi; R10; R11; R08; R09] in
  let k = List.length palette in

  let stack_param_map = ref UidM.empty in
  let reg_param_uids = ref [] in
  
  List.iteri (fun i u ->
    match arg_reg i with
    | None ->
        stack_param_map := UidM.add u (arg_loc i) !stack_param_map
    | Some _ ->
        reg_param_uids := u :: !reg_param_uids
  ) f.f_param;

  let is_stack_param u = UidM.mem u !stack_param_map in
  let move_hints = ref UidM.empty in
  let add_soft_hint u v =
    if is_stack_param u || is_stack_param v then () else
    let s = try UidM.find u !move_hints with Not_found -> UidS.empty in
    move_hints := UidM.add u (UidS.add v s) !move_hints
  in

  let fixed_hints = ref UidM.empty in
  let add_fixed_hint u r =
    if is_stack_param u then () else
    if List.mem r palette then
      let s = try UidM.find u !fixed_hints with Not_found -> [] in
      fixed_hints := UidM.add u (r::s) !fixed_hints
  in

  let build_graph () =
    let g = ref UidM.empty in
    let add_node u = 
      if (not (is_stack_param u)) && (not (UidM.mem u !g)) then 
        g := UidM.add u UidS.empty !g 
    in
    let add_edge_raw u v =
      if u = v || is_stack_param u || is_stack_param v then () else
      let add_one x y = 
        let s = try UidM.find x !g with Not_found -> UidS.empty in
        g := UidM.add x (UidS.add y s) !g
      in add_one u v; add_one v u
    in
    List.iter add_node !reg_param_uids;
    
    List.iteri (fun i u ->
      match arg_reg i with
      | Some r -> add_fixed_hint u r
      | None -> ()
    ) f.f_param;

    let entry, lbl_blocks = f.f_cfg in
    let all_blocks = entry :: List.map snd lbl_blocks in

    List.iter (fun {Ll.insns; term} ->
      List.iter (fun (u, i) -> 
        if insn_assigns i then add_node u;
        
        match i with
        | Ll.Bitcast (_, Ll.Id v, _) 
        | Ll.Load (_, Ll.Id v) -> 
            add_soft_hint u v; add_soft_hint v u
        | Ll.Binop (_, _, Ll.Id v1, Ll.Id v2) ->
            add_soft_hint u v1; add_soft_hint v1 u;
            add_soft_hint u v2; add_soft_hint v2 u
        | Ll.Gep (_, Ll.Id v, _) ->
            add_soft_hint u v; add_soft_hint v u
        ...
        | Ll.Call (_, fo, args) ->
            begin match fo with
            | Ll.Id ptr_u -> 
                add_fixed_hint ptr_u R10; 
                add_fixed_hint ptr_u R11
            | _ -> ()
            end;
            List.iteri (fun i (_, op) ->
              match op, arg_reg i with
              | Ll.Id v, Some r -> add_fixed_hint v r
              | _ -> ()
            ) args
        | _ -> ()
      ) insns
    ...
    let process_block {Ll.insns} =
      List.iter (fun (u, i) ->
        if insn_assigns i then
          UidS.iter (fun v -> add_edge_raw u v) (live.live_out u)
      ) insns
    in
    List.iter process_block all_blocks;

    let entry_u = match entry.insns with (u,_)::_ -> u | [] -> fst entry.term in
    let entry_live = live.live_in entry_u in
    
    let rec param_clique = function
      | [] -> ()
      | p::ps -> List.iter (fun q -> add_edge_raw p q) ps; param_clique ps
    in
    param_clique !reg_param_uids;

    List.iter (fun p ->
      UidS.iter (fun v -> if p <> v then add_edge_raw p v) entry_live
    ) !reg_param_uids;
  ...
  let uses = 
    let counts = ref UidM.empty in
    let inc u = 
      let n = try UidM.find u !counts with Not_found -> 0 in
      counts := UidM.add u (n + 1) !counts 
    in
    List.iter inc f.f_param;
    
    let visit_op = function Ll.Id u -> inc u | _ -> () in
    let visit_block {Ll.insns; term=(_, t)} =
      List.iter (fun (u, i) ->
        if insn_assigns i then inc u;
        match i with
        | Ll.Binop (_,_,o1,o2) | Ll.Store (_,o1,o2) | Ll.Icmp (_,_,o1,o2) -> visit_op o1; visit_op o2
        | Ll.Call (_,o,args) -> visit_op o; List.iter (fun (_,a) -> visit_op a) args
        | Ll.Bitcast (_,o,_) | Ll.Load (_,o) -> visit_op o
        | Ll.Gep (_,o,os) -> visit_op o; List.iter visit_op os
        | _ -> ()
      ) insns;
      match t with Ll.Ret(_, Some o) | Ll.Cbr(o,_,_) -> visit_op o | _ -> ()
    in
    let entry, lbs = f.f_cfg in
    visit_block entry; List.iter (fun (_,b) -> visit_block b) lbs;
    !counts
  in
  let get_use_count u = try UidM.find u uses with Not_found -> 0 in

  let simplify g_init =
    let stack = Stack.create () in
    let removed = ref UidS.empty in
    let nodes = UidM.fold (fun u _ acc -> u::acc) g_init [] in
    let num_nodes = List.length nodes in

    for _ = 1 to num_nodes do
      let candidates = List.filter (fun u -> not (UidS.mem u !removed)) nodes in
      
      let degree u = 
        let neighbors = try UidM.find u g_init with Not_found -> UidS.empty in
        UidS.fold (fun v c -> if UidS.mem v !removed then c else c + 1) neighbors 0
      in

      let spill_candidate = 
        try 
          let u = List.find (fun u -> degree u < k) candidates in
          (u, false)
        with Not_found ->
          let best = List.fold_left (fun (best_u, min_score) u ->
            let deg = degree u in
            let d_float = if deg = 0 then 0.5 else float_of_int deg in
            let score = (float_of_int (get_use_count u)) /. d_float in
            if score < min_score then (Some u, score) else (best_u, min_score)
          ) (None, max_float) candidates in
          match best with
          | (Some u, _) -> (u, true)
          | (None, _) -> (List.hd candidates, true)
      in
      
      let (u, _) = spill_candidate in
      Stack.push u stack;
      removed := UidS.add u !removed
    done;
    stack
  in

  let select g stack =
    let coloring = ref UidM.empty in

    while not (Stack.is_empty stack) do
      let u = Stack.pop stack in
      let neighbors = try UidM.find u g with Not_found -> UidS.empty in
      
      let taken_colors = UidS.fold (fun v acc ->
        match try UidM.find v !coloring with Not_found -> None with
        | Some (Alloc.LReg r) -> r :: acc
        | _ -> acc
      ) neighbors [] in
      let is_forbidden r = List.mem r taken_colors in

      let chosen_color = 
        let hard_hints = try UidM.find u !fixed_hints with Not_found -> [] in
        let valid_hard = List.filter (fun r -> not (is_forbidden r)) hard_hints in
        
        match valid_hard with
        | r::_ -> Some r 
        | [] ->
          let soft_neighbors = try UidM.find u !move_hints with Not_found -> UidS.empty in
          let soft_pref = UidS.fold (fun v acc ->
            match acc with Some _ -> acc | None ->
              match try UidM.find v !coloring with Not_found -> None with
              | Some (Alloc.LReg r) -> 
                  if not (is_forbidden r) && List.mem r palette then Some r else None
              | _ -> None
          ) soft_neighbors None 
          in
          
          match soft_pref with
          | Some r -> Some r
          | None ->
              let rec find_first = function
                | [] -> None
                | r::rs -> if is_forbidden r then find_first rs else Some r
              in
              find_first palette
      in
      
      let loc = match chosen_color with
        | Some r -> Some (Alloc.LReg r)
        | None -> None
      in
      coloring := UidM.add u loc !coloring
    done;
    !coloring
  in

  let stack = simplify g in
  let colors = select g stack in

  let spill_count = ref 0 in
  let final_map = ref !stack_param_map in

  let entry, lbl_blocks = f.f_cfg in
  List.iter (fun (l, _) -> 
    final_map := UidM.add l (Alloc.LLbl (Platform.mangle l)) !final_map
  ) lbl_blocks;

  UidM.iter (fun u res ->
    match res with
    | Some loc -> final_map := UidM.add u loc !final_map
    | None -> 
        incr spill_count;
        final_map := UidM.add u (Alloc.LStk (-(!spill_count))) !final_map
  ) colors;

  { uid_loc = (fun u -> try UidM.find u !final_map with Not_found -> Alloc.LVoid);
